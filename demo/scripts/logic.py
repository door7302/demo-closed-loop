...
try:
    ...
    # --- 3️⃣ Get CM errors ---
    cm_entry = db.cmerrors.find_one({"router_name": cmerror_device}, {"_id": 0})
    cmerrors = cm_entry.get("cmerrors") if cm_entry else None
...
if cmerrors:
    for err in cmerrors:
        if err.get("cmerror_id") == cmerror_id:
            existing_error = err
            break   
  ...
  "cmerrors": [
    {
      ...
      "cmerror_slot": 0,
      "cmerror_pfe": 2,
      "cmerror_desc": "Link flap on xe-0/0/0",
      "cmerror_id": "ALM1234"
    }
"""
...
if existing_error:
    ...
    if cmerror_count > existing_error.get("cmerror_count"):
        ...
        time_diff = (cmerror_update - existing_error.get("cmerror_update")) / 1000  # convert to seconds
        if time_diff < 86400:
        ...
        # Update the cmerror entry in MongoDB
        try:
            db.cmerrors.update_one(
                {"router_name": cmerror_device},
                {"$set": {"cmerrors": {
                    "cmerror_id": cmerror_id,
                    "cmerrors.$.cmerror_count": cmerror_count,
                    "cmerrors.$.cmerror_update": cmerror_update,
                    "cmerrors.$.cmerror_slot": cmerror_slot,
                    "cmerrors.$.cmerror_desc": cmerror_desc,
                    "cmerrors.$.cmerror_pfe": cmerror_pfe
                }
                }}
            )
            LOG.info(f"LOGIC: CMERROR for {cmerror_device} - {cmerror_desc} - UPDATED in MongoDB")
        except Exception as e:
    ...
    # If not exists, create a new cmerror entry in MongoDB
    new_error = {
        "cmerror_id": cmerror_id,
    ...
    if cmerrors is None:
        cmerrors = []
    cmerrors.append(new_error)
    try:
        db.cmerrors.update_one(
            {"router_name": cmerror_device},
            {"$set": {"cmerrors": cmerrors}},
            upsert=True
        )
        LOG.info(f"LOGIC: CMERROR for {cmerror_device} - {cmerror_desc} - NEW ENTRY CREATED")
    except Exception as e:
        LOG.error(f"LOGIC: Unable to create cmerror in MongoDB: {e}")
        raise SystemExit(1)
...
if action_required == 1:
    ...
    LOG.info(f"LOGIC: INTERFACES TO DISABLE on {cmerror_device}: {interfaces_fpc_pfe}")
    
    err = configure_interfaces_disable_state(cmerror_device, interfaces_fpc_pfe, action="disable")
    if err:
    ...
    # Notify NOC team
    LOG.info(f"LOGIC: NOTIFY NOC TEAM - PARTIAL ACTION TAKEN - INTERFACES DISABLED: {interfaces_fpc_pfe}")
...
if action_required == 2:
    ...
    if alarm_desc:
        ...
        if err:
            ...
            raise SystemExit(1) 
        LOG.info(f"LOGIC: INTERFACES DISABLED on {cmerror_device}")
        write_log_to_influx(cmerror_device, f"Disabling interfaces attached on {cmerror_device} and FPC slot {cmerror_slot} and pfe {cmerror_pfe} again due to persistent major alarm", host="influxdb", port=8086, db="demo")
